...
fn has_common_minimal_class_xy(qord:&Vec<Vec<usize>>, min_elems_repr:&HashSet<usize>, x: usize, y: usize) -> bool {
    for t in min_elems_repr {
        if qord[*t][x] == 1 && qord[*t][y] == 1 {
            return true;
        }
    }
    false
...
//     return;
    ...
    for qord2_idx in c_from-1..=c_to-1 {
        ...
        write!(&mut log_filename, "redfalg{}-{:0loglen$}-v8.log", cursize, qord2_idx + 1).unwrap();
        write!(&mut out_filename, "redfalg{}-{:0loglen$}-v8.txt", cursize, qord2_idx + 1).unwrap();
...
//        eprintln!("{}", falglib::rel_count_strict_minimal_elements(&red_qords[qord2_idx]));
//        eprintln!("{:?}", &red_qords[qord2_idx]);
//        eprintln!("{:?}", falglib::rel_get_strict_minimal_classes(&red_qords[qord2_idx]));
//        continue;

        let (cands_set,cands_map) = falglib::rel_get_strict_minimal_classes(&red_qords[qord2_idx]);
...
//        }
        ...
        if cands_set.len() >= 2 {
            ...
            let mut b_just_one_repr = false;
            for t in &cands_set {
                if cands_map[t] == 1 {
                    b_just_one_repr = true;
                    break;
                }
            }
            if b_just_one_repr {
                //eprintln!(">=2, 1cls_repr");
                b_skip = true
            }
        }

        if !b_skip && cands_set.len() >= 2 {
            let v2:Vec<usize> = (cands_map.clone()).into_values().collect();
            if v2[0] != v2[1] {
                b_skip = true;
            }
        }
...
//        if !falglib::rel_strict_minimal_classes_le_one(&red_qords[qord2_idx]) {
            ...
            // writeln!();
            writeln!(&mut log_writer, "Line: {} - {} - Skipping", qord2_idx+1, 0).unwrap();
            writeln!(&mut log_writer, "{}\t{}\t{}\t{}\t{:.4}", qord2_idx+1, 0, 0, 0, 0.00).unwrap();
        ...
        writeln!(&mut log_writer, "{}\t{}\t{}\t{}\t{:.4}", qord2_idx+1, qord2_iso_exp_len, num_compat, num_with_cands, time_iter_start.elapsed().as_secs_f64()).unwrap();
