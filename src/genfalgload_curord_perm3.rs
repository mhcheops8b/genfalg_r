...
// fn has_candidates_check_pair(qord1:&Vec<Vec<usize>>, qord2:&Vec<Vec<usize>>, x: usize, y:usize) -> bool {
//     
//     
//         return true;
//     }
...
fn has_all_candidates_check(qord1:&Vec<Vec<usize>>, qord2:&Vec<Vec<usize>>) -> bool {
    let n = qord1.len();
    for x in 0..n {
        for y in x+1..n {
            if  !has_candidates_check_pair(qord1, qord2, x, y) ||
                !has_candidates_check_pair(qord1, qord2, y, x) {
                    return false;
                }
        }
    }
...
fn main() {
    ...
    // count stricly minimal elements
    // x strictly minimal, if not exist t: t < x
    

    let qord2_ref = &red_qords[curord_idx-1];

    let mut min_count= 0usize;
    for x in 0..qord2_ref.len() {
        let mut bfound = false;
        for t in 0..qord2_ref.len() {
            if t != x && qord2_ref[t][x] == 1 {
                bfound=true;
                break;
            }
        }
    ...
    let iso_expand_full_size = falglib::rel_isomorphic_expand_full_size(&red_qords[curord_idx-1]);            
    let qord2_iso_exp = falglib::rel_isomorphic_expand_reduced_vec(&red_qords[curord_idx-1]).0;
    ...
    for qord2exp_idx in (ordexp_ffrom-1)..=(ordexp_tto-1) {
            ...
            for qord1_idx in 0..curord_idx { 
                ...
                if falglib::rel_are_pair_antisymmetric(&red_qords[qord1_idx], &qord2_iso_exp[qord2exp_idx]) {
                    ...
                    // quick has_candidates check
                    if has_all_candidates_check(&red_qords[qord1_idx], &qord2_iso_exp[qord2exp_idx]) {
                        l_num_with_cands+=1;
                        num_with_cands+=1;
            ...
            l_cnt+=1;
            if l_cnt == iter_limit {
                eprintln!(" - {} - {} - {}", l_num_compat, l_num_with_cands, time_cur_iter_start.elapsed().as_secs_f64());
                l_cnt = 0;
                //l_skip = 0;
            }
    ...
    eprintln!("{}\t{}\t{}\t{}\t{}\t{}", curord_idx, num_items, num_compat, num_with_cands, iso_expand_full_size - qord2_iso_exp_len, time_iter_start.elapsed().as_secs_f64());
