    ...
    if let Ok(lines_falg) = read_lines(&filename) {
        for line_falg in lines_falg.map_while(Result::ok) {
            ...
            if falglib::falg_all_tests_no_print(&parsed_falg) {
                if !falglib::falg_is_min_repr(&parsed_falg) {
                    println!("Err (repr): {:?} -> {:?}", parsed_falg, falglib::falg_find_min_repr(&parsed_falg));
                    //all_falg.insert(parsed_falg);
                }
            }
            else {
                println!("Err (falg): {:?}", parsed_falg);
            }

            if processed_cnt % 1000 == 1 {
                eprintln!("{}", processed_cnt);
            }
        }
        eprintln!("Total processed: {}", processed_cnt);
    }
    else {
