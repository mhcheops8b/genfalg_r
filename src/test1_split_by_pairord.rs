...
fn main() {
    ...
    if args_len < 3 {
        println!("Usage: {} <size> <falg_res_file>", std::env::args().next().unwrap());
        return;
    }
    ...
    if let Ok(lines_qord) = read_lines(&res_filename) {
        for line_qord in lines_qord.map_while(Result::ok) {
            if line_qord.chars().nth(0).unwrap() == '#' {
                continue;
            }
            // line_no+=1;
            let parsed_falg = falglib::parse_vector(cursize, &line_qord);
            if falglib::falg_is_associative(&parsed_falg) {
                print!("+A");
            }
            else {
                print!("-A");
                if falglib::falg_test_cond_n2(&parsed_falg, false) {
                    print!(", +N2")
                }
                else {
                    print!(", -N2");
                ...
                if falglib::falg_test_cond_n3(&parsed_falg, false) {
                    print!(", +N3")
                }
                else {
                    print!(", -N3");
                ...
                if falglib::falg_test_cond_n4(&parsed_falg, false) {
                    print!(", +N4")
                }
                else {
                    print!(", -N4");
                ...
                if falglib::falg_test_cond_n5(&parsed_falg, false) {
                    print!(", +N5")
                }
                else {
                    print!(", -N5");
                ...
                println!("{:?}", falglib::falg_associativity_problems(&parsed_falg));
                // if falglib::falg_test_f5(&parsed_falg, false) {
                //     print!(", +F5");
                // }
                // else {
                //     print!(", -F5");
                // }
                // if falglib::falg_test_f6(&parsed_falg, false) {
                //     print!(", +F6");
                // }
                // else {
                //     print!(", -F6");
            ...
            // let cur_qord2 = falglib::falg_get_qord2(&parsed_falg);
